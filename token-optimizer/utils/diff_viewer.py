...
class DiffViewer:
    ...
    def _show_side_by_side(self, text1: str, text2: str, stage_name: str):
        ...
        table.add_row(
            Panel(before_text, border_style="red"),
            Panel(after_text, border_style="green")
        )
    ...
    def _show_stats(self, stats: Dict[str, Any], stage_name: str):
        ...
        stats_table = Table(title=f"{stage_name} Statistics", show_header=True, header_style="bold cyan")
    ...
    def show_inline_diff(self, text1: str, text2: str, stage_name: str):
        ...
        self.console.print(Panel(
            '\n'.join(panel_content),
            title=f"{stage_name} Changes",
            border_style="blue"
        ))
    
    def show_token_analysis(self, original_tokens: int, current_tokens: int, 
                           stage_tokens: List[Tuple[str, int]], total_time: float):
    ...
    def show_replacements(self, replacements: List[Dict[str, Any]], stage_name: str):
        ...
        table = Table(title=f"{stage_name} Replacements", show_header=True, header_style="bold yellow")
        ...
        sorted_replacements = sorted(replacements, 
                                   key=lambda x: x.get('tokens_saved', 0), 
                                   reverse=True)
        ...
        if len(replacements) > 10:
            table.add_row(
                f"[dim]... and {len(replacements) - 10} more[/dim]",
                "", "", "",
                style="dim"
            )
