...
class TokenAwareOptimizer:
    ...
    def optimize_text(self, text):
        ...
        sorted_replacements = sorted(
            self.token_efficient_map.items(),
...
def main():
    ...
    parser.add_argument('-s', '--min-savings', type=int, default=1,
                       help='Minimum token savings required (default: 1)')
    parser.add_argument('-a', '--analyze', action='store_true',
                       help='Analyze only, don\'t optimize')
    ...
    if args.analyze:
        ...
        if args.verbose:
            ...
            print(f"Reduction: {(stats['total_tokens_saved'] / stats['original_tokens'] * 100):.1f}%", file=sys.stderr)
            ...
            if stats['replacements']:
                ...
                for repl in stats['replacements']:
                    print(f"  '{repl['original']}' → '{repl['replacement']}': "
                          f"{repl['count']} times, saved {repl['tokens_saved']} tokens", file=sys.stderr)
