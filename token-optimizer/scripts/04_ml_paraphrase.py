...
def main():
    ...
    parser.add_argument('-m', '--model', default='t5-small',
                       help='Model to use (default: t5-small)')
    parser.add_argument('-r', '--ratio', type=float, default=0.8,
                       help='Target length ratio (default: 0.8 = 80% of original)')
    parser.add_argument('-l', '--llmlingua', action='store_true',
                       help='Try to use LLMLingua if available')
    ...
    if args.verbose:
        ...
        print(f"Original: {stats['original_length']} chars, {stats['original_tokens']} tokens", file=sys.stderr)
        print(f"Compressed: {stats['compressed_length']} chars, {stats['compressed_tokens']} tokens", file=sys.stderr)
