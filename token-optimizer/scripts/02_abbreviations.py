...
def main():
    ...
    parser.add_argument(
        "-c",
        "--config",
        default="config/abbreviations.json",
        help="Path to abbreviations config file",
    )
    parser.add_argument("-v", "--verbose", action="store_true", help="Show replacement statistics")
    ...
    if args.verbose and replacements:
        ...
        for long_form, short_form, count in sorted(replacements, key=lambda x: x[2], reverse=True)[
            :10
        ]:
