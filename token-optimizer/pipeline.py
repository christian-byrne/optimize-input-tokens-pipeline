...
class TokenOptimizationPipeline:
    ...
    def run_stage(self, stage_name, script_name, input_text, extra_args=None):
        ...
        try:
            ...
            print(f"✓ {stage_name}: {input_tokens} → {output_tokens} tokens "
                  f"(-{input_tokens - output_tokens}, {elapsed:.2f}s)", file=sys.stderr)
    ...
    def run_pipeline(self, input_text):
        ...
        print(f"Total reduction: {original_tokens - final_tokens} tokens "
              f"({(1 - final_tokens/original_tokens) * 100:.1f}%)", file=sys.stderr)
...
Examples:
  ...
  python pipeline.py input.txt --count-only
    ...
    if args.verbose and VERBOSE_AVAILABLE:
        ...
        print("Warning: Verbose mode requires 'rich' library. Install with: pip install rich", file=sys.stderr)
