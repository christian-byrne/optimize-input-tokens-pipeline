...
class VerboseTokenOptimizationPipeline:
    ...
    def run_stage(self, stage_name, script_name, input_text, extra_args=None):
        ...
        if not self.config['pipeline'][stage_name]['enabled']:
            if self.verbose:
                self.console.print(f"[yellow]⏭️  Skipping {stage_name} (disabled in config)[/yellow]")
            else:
        ...
        if self.verbose:
            with self.console.status(f"[bold green]Running {stage_name}...[/bold green]", spinner="dots"):
        ...
        if self.verbose:
            ...
            print(f"✓ {stage_name}: {input_tokens} → {output_tokens} tokens "
                  f"(-{tokens_saved}, {elapsed:.2f}s)", file=sys.stderr)
    ...
    def run_pipeline(self, input_text):
        ...
        if self.config['pipeline']['token_aware']['enabled']:
            current_text = self.run_stage(
                ...
                '03_token_aware.py',
                current_text,
                ['-m', self.config['tokenizer']['model'],
                 '-s', str(self.config['pipeline']['token_aware']['min_token_savings'])]
            )
        ...
        if self.config['pipeline']['ml_paraphrase']['enabled']:
            current_text = self.run_stage(
                ...
                '04_ml_paraphrase.py',
                current_text,
                ['-m', self.config['pipeline']['ml_paraphrase']['model'],
                 '-r', str(self.config['pipeline']['ml_paraphrase']['max_length_ratio'])]
            )
        ...
        if self.verbose:
            ...
            print(f"Total reduction: {total_reduction} tokens ({reduction_pct:.1f}%)", file=sys.stderr)
        ...
        if result.returncode == 0:
            ...
            if self.verbose and optimizations:
                ...
                for opt in optimizations[:10]:  # Top 10
                    print(f"  '{opt['phrase']}' → '{opt['suggested']}': "
                          f"{opt['occurrences']}x, save {opt['total_savings']} tokens", 
                          file=sys.stderr)
                print(f"\nTotal potential savings from top 10: {total_savings} tokens", file=sys.stderr)
            else:
...
Examples:
  ...
  python pipeline_verbose.py input.txt --analyze --verbose
    ...
    parser.add_argument('-v', '--verbose', action='store_true',
                       help='Show detailed diffs and statistics')
    parser.add_argument('-a', '--analyze', action='store_true',
                       help='Analyze optimization potential without processing')
    parser.add_argument('--count-only', action='store_true',
                       help='Only show token count, don\'t process')
